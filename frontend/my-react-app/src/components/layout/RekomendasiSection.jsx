...
const RekomendasiSection = () => {
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      // Handle specific error from API about incomplete profile
      if (err.message && (
        err.message.includes("Interest and address must be set in profile") ||
        ...
        // Show profile alert when API specifically says profile is incomplete
        const hasShownAlert = localStorage.getItem('profileAlertShown');
        if (!hasShownAlert) {
          setShowProfileAlert(true);
          localStorage.setItem('profileAlertShown', 'true');
        ...
        let errorMessage = "Gagal memuat rekomendasi wisata. Pastikan koneksi internet Anda stabil dan coba lagi.";
        
        if (err.message.includes('401') || err.message.includes('Unauthorized')) {
          errorMessage = "Sesi Anda telah berakhir. Silakan login kembali.";
        } else if (err.message.includes('403') || err.message.includes('Forbidden')) {
          errorMessage = "Akses ditolak. Pastikan profil Anda sudah lengkap.";
        } else if (err.message.includes('404')) {
          errorMessage = "Layanan rekomendasi tidak tersedia saat ini.";
      ...
      <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6 leading-relaxed">
            ...
            silakan lengkapi terlebih dahulu informasi <strong>domisili</strong> dan <strong>minat wisata</strong> di bagian profil.
          </p>
